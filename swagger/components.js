  ...
  schemas: {
    ...
    RegisterBody: {
      ...
      properties: {
        ...
        schoolTrack: {
          type: "string",
          enum: ["primary_school", "middle_school", "high_school"],
        ...
        schoolGrade: {
          ...
          maximum: 6,
          description:
            "Year within the track. `primary_school`: 1–6; `middle_school` / `high_school`: 1–3. Required when `educationStatus` is `school`.",
        },
        universityYear: {
          type: "integer",
          minimum: 1,
    ...
    OauthGoogleBody: {
      ...
      properties: {
        ...
        schoolTrack: { type: "string", enum: ["primary_school", "middle_school", "high_school"] },
        schoolGrade: { type: "integer", minimum: 1, maximum: 6 },
    ...
    UpdateMeBody: {
      ...
      properties: {
        ...
        schoolGrade: {
          ...
          maximum: 6,
          nullable: true,
          description: "`primary_school`: 1–6; `middle_school` / `high_school`: 1–3.",
        },
      ...
      description: [
        ...
        "Education (same rules as signup): when updating any education field, send **`educationStatus`** plus the fields for that level — `school` → `schoolTrack` + `schoolGrade` (`primary_school` grades 1–6; `middle_school` / `high_school` grades 1–3); `university` → `universityYear`; `other` → `educationOtherDetail`.",
    ...
    AiMaterialSourceType: {
      type: "string",
      description: "Client may send `ai`; stored as `ai_gemini`. Also: `shared_copy`, `tutor`, `upload_model_response`.",
      enum: ["ai", "ai_gemini", "shared_copy", "tutor", "upload_model_response"],
      example: "ai",
    },
    QuizCreateBody: {
      type: "object",
      ...
      properties: {
        ...
        sourceType: { $ref: "#/components/schemas/AiMaterialSourceType" },
        totalQuestions: { type: "integer", minimum: 0, maximum: 100 },
        chatSessionId: { type: "string", format: "uuid" },
        sourceReference: { type: "string", maxLength: 255, nullable: true },
        savedAt: { type: "string", format: "date-time", nullable: true },
      },
      example: { title: "Generated Quiz", sourceType: "ai", totalQuestions: 10 },
    },
    QuizQuestionCreateBody: {
      type: "object",
      additionalProperties: false,
      required: ["quizId", "questionText"],
      description: "Use **either** `optionA`–`optionD` + `correctOption` **or** `options[]` + `correctOptionIndex`.",
      properties: {
        quizId: { type: "string", format: "uuid" },
        questionText: { type: "string", minLength: 1, maxLength: 8000 },
        optionA: { type: "string", maxLength: 2000 },
        optionB: { type: "string", maxLength: 2000 },
        optionC: { type: "string", maxLength: 2000 },
        optionD: { type: "string", maxLength: 2000 },
        correctOption: { type: "string", enum: ["A", "B", "C", "D"] },
        options: { type: "array", items: { type: "string" }, minItems: 2, maxItems: 4 },
        correctOptionIndex: { type: "integer", minimum: 0, maximum: 3 },
        order: { type: "integer", minimum: 0, description: "Accepted but not persisted; order by createdAt." },
      },
      example: {
        quizId: Ex.UUID,
    ...
    FlashcardSetCreateBody: {
      ...
      example: { title: "Generated Flashcards", sourceType: "ai" },
    },
    FlashcardNestedCreateBody: {
      type: "object",
      additionalProperties: false,
      ...
      example: { question: "[term] Mitochondria", answer: "Powerhouse of the cell" },
    },
    SummaryCreateBody: {
      type: "object",
      additionalProperties: false,
      required: ["summaryText", "sourceType"],
      properties: {
        summaryText: { type: "string", minLength: 1, maxLength: 100000, description: "Plain text body (not `content` or `raw`)." },
        sourceType: { $ref: "#/components/schemas/AiMaterialSourceType" },
        chatSessionId: { type: "string", format: "uuid" },
        sourceReference: { type: "string", maxLength: 255, nullable: true },
        savedAt: { type: "string", format: "date-time", nullable: true },
      },
      example: { summaryText: "Chapter 3 recap…", sourceType: "ai" },
    },
    MindMapTreeContent: {
      type: "object",
      description: "Stanley AI shape: `{ title, children[] }`. Stored in DB `content` column; top-level `title` duplicated for search.",
      ...
      properties: {
        ...
        children: {
          ...
          description: "Nested nodes; each node may include `title`, `children`, etc.",
        },
      },
      example: {
        title: "Exam topics",
    ...
    MindMapCreateBody: {
      ...
      required: ["sourceType"],
      description:
        "Send `content: { title, children[] }` (Stanley shape). Top-level `title` optional if `content.title` set. Do **not** send `raw`.",
      properties: {
        title: { type: "string", minLength: 1, maxLength: 500 },
        content: { $ref: "#/components/schemas/MindMapTreeContent" },
        children: {
          type: "array",
          items: { type: "object", additionalProperties: true },
          description: "Alternative: root-level `title` + `children` (merged into `content`).",
        },
        sourceType: { $ref: "#/components/schemas/AiMaterialSourceType" },
        chatSessionId: { type: "string", format: "uuid" },
        sourceReference: { type: "string", maxLength: 255, nullable: true },
    ...
    UserPreferenceReportEmailBody: {
      ...
      properties: {
        ...
        reportEmailRecipient: {
          ...
          format: "email",
          nullable: true,
          description: "Custom recipient for scheduled auto-email. `null` = use account email.",
        },
      },
      example: {
    ...
    UserPreferenceReportEmailSettings: {
      type: "object",
      additionalProperties: false,
      description: "Nested under `data.reportEmail` on GET/PATCH `/user-preferences/me`.",
      properties: {
        reportEmailAutoEnabled: { type: "boolean" },
        reportEmailFrequency: { type: "string", enum: ["weekly", "monthly"], nullable: true },
        reportEmailLocale: { type: "string", enum: ["en", "ar"] },
        accountEmail: {
          type: "string",
          format: "email",
          description: "Signup/login email — use for “send to my email” UI option.",
        },
        reportEmailRecipient: {
          type: "string",
          format: "email",
          nullable: true,
          description: "Saved custom address; `null` when using account email.",
        },
        usingCustomRecipient: {
          type: "boolean",
          description: "True when `reportEmailRecipient` is set (custom option selected).",
        },
        effectiveRecipientEmail: {
          type: "string",
          format: "email",
          description: "Address used for scheduled auto-send: custom if set, else account.",
        },
        reportEmailLastSentAt: { type: "string", format: "date-time", nullable: true },
        reportEmailLastPeriodKey: { type: "string", nullable: true, example: "week:2026-05-19" },
      },
    },
    YourReportExportBody: {
      type: "object",
      additionalProperties: false,
      ...
      properties: {
        ...
        emailRecipient: {
          ...
          enum: ["account", "custom"],
          description:
            "`account` = signup email. `custom` = requires `emailTo`. Omit to use `emailTo`, then saved `reportEmailRecipient`, then account email.",
        },
        emailTo: {
          type: "string",
          format: "email",
          description: "Required when `emailRecipient` is `custom`.",
        },
    ...
    YourReportExportResult: {
      ...
      properties: {
        ...
        pdfUrl: { type: "string", format: "uri", description: "Signed download URL (includes `token` query param)." },
        expiresAt: { type: "string", format: "date-time" },
        emailedTo: { type: "string", format: "email", nullable: true },
    ...
    StudyReportExportBody: {
      ...
      properties: {
        ...
        emailRecipient: {
          type: "string",
          enum: ["account", "custom"],
          description: "account = signup email; custom = requires emailTo.",
        },
        emailTo: { type: "string", format: "email", description: "Required when emailRecipient is custom." },
        timezone: { type: "string", example: "Africa/Cairo" },
      },
    ...
    CommunityChannelMessage: {
      ...
      properties: {
        ...
        communityChannelId: { type: "string", format: "uuid" },
        ...
        senderName: {
          type: "string",
          nullable: true,
          description: "Author display name (alias: senderDisplay).",
          example: "Norhan Mohamed",
        },
        senderDisplay: {
          type: "string",
          nullable: true,
          description: "Same as senderName; preferred for Flutter chat UI.",
          example: "Norhan Mohamed",
        },
        senderUserId: { type: "string", format: "uuid", description: "Same as userId." },
        senderRole: {
          type: "string",
          enum: ["owner", "admin", "member"],
          description: "Author's role in the parent community (for chat badges).",
          example: "member",
        },
      },
    },
    CommunityInviteMemberBody: {
      type: "object",
      additionalProperties: false,
      properties: {
      ...
      description: "Provide exactly one of email or username.",
      example: { email: "friend@example.com" },
    },
    ...
    StudentSubjectCreateBody: {
      ...
      example: { name: "Physics", color: "#4E7DBA" },
    },
    StudentSubjectUpdateBody: {
      type: "object",
      additionalProperties: false,
      properties: {
        name: { type: "string", minLength: 1, maxLength: 100 },
