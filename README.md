...
- **Reports:** **`GET …/sessions/:id/full-report`** — **`exportMeta`** (truncation / totals), **session** (+ **`timerState`**), **`catalogSnapshot`** (preset + embedded **`customPreset`** when used), **`linkedTask`** (when **`linkedTaskId`** set), **checkIns**, **activityEvents**, **mlReports**, **`computed`** (telemetry window, **5‑minute activity buckets**, **lifecycle timeline**, check-in summaries, heuristic **ML rollups**, goals vs planned). **`GET /study-with-mishka/reports`** — paginated full-report-shaped **`items[]`**. **Period rollups (UTC):** **`GET /study-with-mishka/reports/day?date=YYYY-MM-DD`**, **`GET …/reports/week?date=YYYY-MM-DD`** (week containing that date, Monday UTC), **`GET …/reports/month?year=&month=`**, **`GET …/reports/year?year=YYYY`** (12 monthly buckets) — **`totals`**, per-session **`sessionSummaries`** (day/week/month only), **`userContext`** (daily streak snapshot, completed count, baseline avg main-study from last **90** completed/abandoned sessions vs period average). Optional **`topLevelMode`**: `concentration`, `call_with_mishka`, or **`all`** / omit for both modes combined. See **`docs/YOUR_REPORT_BACKEND.md`**.
...
- **Your Report (Flutter handoff):** [`docs/FLUTTER_YOUR_REPORT_HANDOFF.md`](docs/FLUTTER_YOUR_REPORT_HANDOFF.md). APIs: **`GET /reports/your-report`**, **`POST /reports/your-report/export`**, **`GET /user-preferences/me`** (auto email), study/AI/tasks/streak/community report routes.
...
**Tasks:** **`title`** is required; **`listId`** is **optional** — omit it to create a standalone task not attached to any todo list. Other optional fields: **`description`**, **`dueDate`** + **`dueTime`** (UTC calendar semantics on the wire), **`status`**: **`pending`**, **`completed`**, **`missed`**, **`priority`**: **`low`**, **`medium`**, **`high`**, **`taskType`**: **`online`**, **`offline`**. **`completedAt`** is set when status becomes **`completed`** and cleared when reopened. Use **`PATCH /tasks/:id`** (or **`PUT`**) to update — send only changed fields, e.g. `{ "status": "completed" }`. **`GET /tasks/report/completions?from=&to=&granularity=day`** — daily completion counts for Your Report. **`DELETE /tasks/:id`** removes the task. **`PATCH /todo-lists/:id`** (or **`PUT`**) to rename / update a list.
